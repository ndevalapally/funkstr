...
-(void)connect:(NSString*)userName password:(NSString*)password host:(NSString*)host boshURL:(NSString*)boshURL
{
    ...
    if(!self.isConnected)
    {
        ...
        NSDictionary *attributes = @{@"content": @"text/xml; charset=utf-8",
                                     ...
                                     @"from": userName , //[NSString stringWithFormat:@"%@@%@",userName,host]
                                     @"to": host,
                                     @"window": @"5",
                                     @"wait": @"5",
                                     @"xmlns": XMLNS_BOSH,
...
-(void)getRoster
{
    ...
    [element.children addObject:query];
...
-(void)getBookmarks
{
    ...
    [query.children addObject:storage];
    [self addContent:element];
}
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(self.optCount < self.maxCount-1) //for the listen connection
    {
        if(self.contentQueue.count > 0)
        {
            ...
            for(XMLElement *content in self.contentQueue)
                [body.children addObject:content];
            [self.contentQueue removeAllObjects];
    ...
    if(!self.isConnected)
    {
        ...
        if(bind)
        {
            ...
            if(jidElement)
            {
                ...
                [element.children addObject:sesson];
                [self addContent:element];
...
//                [element.children addObject:bind];
//                [self addContent:element];
                // Added content
    ...
    else
    {
        ...
        else if(element.children.count > 0 && [element.name isEqualToString:@"body"])
        {
            for(XMLElement* response in element.children)
            {
...
-(void)handleMessageResponse:(XMLElement*)element
{
    ...
    if(user)
    {
        ...
        if(body && body.text)
        {
            NSString *text = [body.text stripXMLTags];
            if(group)
            {
...
-(void)handleVCardResponse:(XMLElement*)element
{
    ...
    if(user)
    {
        ...
        if(nameElement && nameElement.text.length > 0)
            user.nickname = [nameElement.text stripXMLTags];
...
-(void)handlePresenceResponse:(XMLElement*)element
{
    ...
    if(user)
    {
        ...
        if(statusElement)
        user.status = [statusElement.text stripXMLTags];//;[statusElement.text xmlUnSafe];
        else
            user.status = nil;
...
-(XMLElement*)buildPresence:(DCUserPresence)presence status:(NSString*)status to:(NSString*)jid
{
    ...
    if(presence != DCUserPresenceUnAvailable)
    {
        ...
        [element.children addObject:proElement];
    }
    ...
    if(presence != DCUserPresenceAvailable)
    {
        ...
        //else
        //    show.text = @"chat";
        [element.children addObject:show];
    }
    ...
    if(status)
    {
        ...
        else
        {
            ...
            [element.children addObject:statusElement];
        }
    }
    if(self.currentUser.avatarData && presence != DCUserPresenceAvailable)
    {
        ...
        [xElement.children addObject:photoElement];
        [element.children addObject:xElement];
    }
